...
int thread_insert_inv3(my_dict_t* s, vector<const char*> v, int start, int end) {
  for(size_t i = start; i < end; i++) {
    dict_set(s, v[i], i+1);
  }
  return 1;
...
TEST(DictTest, Invariant2) {
  ...
  future<int> results[4];
  for(size_t i = 0; i < num_threads; i++) {
    results[i] = async(thread_get, &s, different_v, num_entry*i, num_entry*(i+1));
  }
  int final_sum = 0;
  for(size_t i = 0; i < num_threads; i++) {
    int res = results[i].get();
...
TEST(DictTest, Invariant3) {
  ...
  for(size_t i = 0; i < num_threads; i++) {
    t[i] = thread(thread_insert_inv3, &s, v, num_entry*i, num_entry*(i+1));
  }
